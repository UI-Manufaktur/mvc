...
string setSessionStorage(string[] keys) {
  string result;
  foreach (key; keys)
    result ~= "sessionStorage.setItem('" ~ key ~ "', " ~ key ~ ");";
  return result;
}

string setSessionStorage(STRINGAA data) {
  string result;
  foreach (key; data.byKey)
    result ~= "sessionStorage.setItem('" ~ key ~ "', '" ~ data[key] ~ "');";
  return result;
}
...
auto jsDoSelectEntities() {
  return jsFunc("doSelectEntities", ["event"],
    jsLet("selected", "cbSelectAllEntities.checked") ~
      ...
      jsLet(
        "filteredEntities", "filterEntities(entities)") ~
...
auto jsEditEntity() {
  return jsFunc("editEntity", [],
    jsLet("alerts", "''") ~
      ...
      jsIfElse(
        jsOr("entity_name.value.length===0", "entity_display.value.length===0"),//  Name or Display missing
        "alerts += checkEntityName(entity_name);" ~
        "alerts += checkEntityDisplay(entity_display);" ~
        "messages.innerHTML=alerts;",// Sending Form Data to server
...
auto jsCreateEntity() {
  return jsFunc("createEntity", [],// Init values
    jsLet("alerts", "''") ~
      "messages.innerHTML = '';" ~
      "entity_name.classList.remove('is-valid');" ~
      "entity_name.classList.remove('is-invalid');" ~
...
auto jsDeleteEntity() {
  return jsFunc("deleteEntity", [],
    "messages.innerHTML = '';" ~
      jsElementById(["Form"]) ~
      jsConst("formData", "new FormData(Form)") ~
      jsFetch("myApi+'/delete'", [
          "method": "'POST'",
          "sessionId": "sessionId",
          "body": "formData"
        ], [
...
auto jsViewEntity() {
  return jsFunc("viewEntity", ["formId"],
    "messages.innerHTML = '';" ~
      jsConst("formData", "new FormData(Form)") ~
      jsFetch("myApi+'/view'", [
          "method": "'POST'",
