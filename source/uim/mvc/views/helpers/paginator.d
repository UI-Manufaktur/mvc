...
class PaginatorHelper : DMVCHelper {
  ...
  StringTemplate templater() {
     ...
     */
    string sortKey(Nullable!string myModel = null, array myOptions = null) {
        if (empty(myOptions)) {
    ...
    string sortDir(Nullable!string myModel = null, array myOptions = null) {
        ...
        if (!empty(myOptions["direction"])) {
            $dir = myOptions["direction"]).toLower;
        }
    ...
    string sort(string myKey, $title = null, array myOptions = null) {
        ...
        $defaultDir = isSet(myOptions, "direction") ? myOptions["direction"].toLower : "asc";
        ...
        return _templater().format(myTemplate, $vars);
    }
    ...
    array generateUrlParams(array myOptions = null, Nullable!string myModel = null, array myUrl = null) {
        ...
        if (
            isSet($paging, "sortDefault", "directionDefault") && isSet(myOptions, "sort", "direction")
            && myOptions["sort"] == $paging["sortDefault"]
            && myOptions["direction"]).toLower == $paging["directionDefault"]).toLower
        ) {
    ...
    string counter(string format = "pages", array myOptions = null) {
        ...
        switch ($format) {
            ...
            default:
                ...
                _templater().add([myTemplate: $format]);
        }
        ...
        $map += [
            "model": Inflector::humanize(Inflector::tableize(myOptions["model"]))).toLower,
        ];

        return _templater().format(myTemplate, $map);
    }
     ...
     *
     * @param uim.mvc.views\StringTemplate myTemplater StringTemplate instance.
     * @param array<string, mixed> myOptions Options from the numbers() method.
     * @return string
     */
